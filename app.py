...
.metric-card{background:#161b22;border:1px solid #30363d;border-radius:10px;padding:16px 20px;margin-bottom:8px}
.metric-label{font-size:11px;color:#6e7681;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px}
.metric-value{font-size:24px;font-weight:700;font-family:monospace}
.metric-sub{font-size:11px;color:#8b949e;margin-top:4px}
.green{color:#3fb950}.blue{color:#58a6ff}.amber{color:#ffa657}.red{color:#f78166}.purple{color:#d2a8ff}
.info-box{background:rgba(88,166,255,.08);border:1px solid rgba(88,166,255,.25);border-radius:8px;padding:10px 14px;font-size:13px;color:#58a6ff;margin-bottom:12px}
</style>""", unsafe_allow_html=True)

# ── SEED DATA (all IDs are strings) ──────────────────────────────────────────
SEED_ITEMS = [
...
# ── INIT SESSION STATE (called immediately, before anything else) ─────────────
import copy

if "items"   not in st.session_state: st.session_state.items   = copy.deepcopy(SEED_ITEMS)
if "pricing" not in st.session_state: st.session_state.pricing = copy.deepcopy(SEED_PRICING)
if "invoices"not in st.session_state: st.session_state.invoices= copy.deepcopy(SEED_INVOICES)
if "actuals" not in st.session_state: st.session_state.actuals = copy.deepcopy(SEED_ACTUALS)
if "fx"      not in st.session_state: st.session_state.fx      = {"purchase":26.5,"pricing":27.0,"reporting":27.2}
if "season"  not in st.session_state: st.session_state.season  = "2025"
...
def get_item(item_id):
    iid = str(item_id)
    return next((i for i in st.session_state.items if str(i["id"]) == iid), {})

def fmt_usd(v): return f"${float(v or 0):,.2f}"
def fmt_zmw(v): return f"K{float(v or 0):,.2f}"
def fmt_n(v, d=2): return f"{float(v or 0):,.{d}f}"

def calc_selling(itype, cost, markup_pct):
    return cost if itype == "recovery" else cost * (1 + markup_pct / 100)

def calc_pricing(p):
    item    = get_item(p["item_id"])
    cost    = float(p["cost_usd"])
    rfx     = float(st.session_state.fx["reporting"])
    selling = calc_selling(item.get("type","markup"), cost, float(p["markup_pct"]))
    margin  = 0.0 if item.get("type") == "recovery" else selling - cost
    qty     = float(p["qty"])
    return {
        "cost_zmw":     cost * float(p["fx_rate"]),
        "selling_usd":  selling,
        "selling_zmw":  selling * rfx,
        "margin_unit":  margin,
        "total_margin": margin * qty,
        "total_rev":    selling * qty,
        "margin_pct":   (margin / selling * 100) if selling > 0 else 0.0,
    }

def calc_invoice(inv):
    gross   = float(inv["gross"])
    vat     = float(inv["vat"])
    net     = gross if inv["vat_rec"] else gross + vat
    qty     = float(inv["qty"])
    pfx     = float(st.session_state.fx["purchase"])
    return {
        "net":    net,
        "cpu":    net / qty if qty > 0 else 0.0,
        "usd_eq": gross if inv["currency"] == "USD" else net / pfx,
    }

def calc_actual(a):
    rfx     = float(st.session_state.fx["reporting"])
    zmw     = float(a["amount_zmw"])
    qty     = float(a["qty"])
    aid     = str(a["item_id"])
    inv     = next((i for i in st.session_state.invoices
                    if str(i["item_id"]) == aid and i["currency"] == "USD"), None)
    return {
        "cpu_zmw":     zmw / qty if qty > 0 else 0.0,
        "cost_usd":    zmw / rfx if rfx > 0 else 0.0,
        "implied_fx":  (zmw / float(inv["gross"])) if inv and float(inv["gross"]) > 0 else None,
        "matched_inv": inv,
    }

def get_metrics():
    rfx = float(st.session_state.fx["reporting"])
    rev = comm = actual_zmw = 0.0
    for p in st.session_state.pricing:
        c    = calc_pricing(p)
        rev  += c["total_rev"]
        comm += c["total_margin"]
    for a in st.session_state.actuals:
        actual_zmw += float(a["amount_zmw"])
    act_usd = actual_zmw / rfx if rfx > 0 else 0.0
    act_mar = rev - act_usd
    return {"revenue":rev,"comm_margin":comm,"actual_margin":act_mar,
            "variance":comm - act_mar,"actual_cost_usd":act_usd}
...
with st.sidebar:
    ...
    page = st.radio("Navigation", [
        "📊 Dashboard","📦 Input Master","💰 Commercial Pricing",
        "🧾 Invoice Capture","📋 Sage Actuals",
        "💱 FX Rates","⚖️ Reconciliation","📈 Reports",
    ])
    st.divider()
...
if page == "📊 Dashboard":
    ...
    m   = get_metrics()
    rfx = float(st.session_state.fx["reporting"])
    pct = (m["comm_margin"] / m["revenue"] * 100) if m["revenue"] > 0 else 0.0

    c1,c2,c3,c4 = st.columns(4)
    with c1: metric_card("Total Revenue",     fmt_usd(m["revenue"]),       "Commercial selling value","blue")
    with c2: metric_card("Commercial Margin", fmt_usd(m["comm_margin"]),   f"{fmt_n(pct)}% margin","green")
    with c3: metric_card("Actual Margin",     fmt_usd(m["actual_margin"]), "Post Sage reconciliation","amber")
    s = "▲" if m["variance"] >= 0 else "▼"
    with c4: metric_card("FX / Proc Variance",f"{s} {fmt_usd(abs(m['variance']))}",
                         "vs commercial expectation","green" if m["variance"]>=0 else "red")
    ...
    with c1:
        ...
        }, index=["Revenue","Comm Margin","Actual Margin","FX/Proc Var"]))
    with c2:
        st.markdown("#### FX Rates Comparison")
        st.bar_chart(pd.DataFrame({
            "ZMW/USD":[fx["purchase"],fx["pricing"],fx["reporting"]]
        }, index=["Purchase FX","Pricing FX","Reporting FX"]))
    ...
    for p in st.session_state.pricing:
        item   = get_item(p["item_id"])
        c      = calc_pricing(p)
        actual = next((a for a in st.session_state.actuals
                       if str(a["item_id"]) == str(p["item_id"])), None)
        ...
        rows.append({
            ...
            "FX/Proc Var":     (("▲ " if variance>=0 else "▼ ")+fmt_usd(abs(variance)))
...
# ── INPUT MASTER ──────────────────────────────────────────────────────────────
    ...
    with st.expander("➕ Add New Item"):
        with st.form("add_item"):
            ...
            with c1:
                ...
                cat   = st.selectbox("Category",["Fertilizer","Chemicals","Seed","Packaging","Fuel","Inputs"])
            with c2:
                itype  = st.selectbox("Type",["markup","recovery"])
            ...
            if st.form_submit_button("Save Item", type="primary"):
                if not code or not name:
                    ...
                    st.session_state.items.append(
                        {"id":nid(),"code":code,"name":name,"category":cat,
                         "type":itype,"markup":markup,"uom":uom})
    ...
    st.dataframe(pd.DataFrame([{
        ...
        "UoM":i["uom"],
    } for i in st.session_state.items]), use_container_width=True, hide_index=True)
...
# ── COMMERCIAL PRICING ────────────────────────────────────────────────────────
    ...
    with st.expander("➕ Add / Replace Pricing"):
        with st.form("add_pricing"):
            ...
            with c2:
                fxr  = st.number_input("Pricing FX Rate",value=float(st.session_state.fx["pricing"]),step=0.001,format="%.3f")
            ...
            margin  = 0.0 if item.get("type")=="recovery" else selling - cost
            st.info(f"**Preview** → Selling: {fmt_usd(selling)} | Margin/unit: {fmt_usd(margin)} | Total: {fmt_usd(margin*qty)}")

            if st.form_submit_button("Save Pricing", type="primary"):
                st.session_state.pricing = [p for p in st.session_state.pricing
                                            if str(p["item_id"]) != str(item_id)]
                st.session_state.pricing.append(
                    {"id":nid(),"item_id":str(item_id),"cost_usd":cost,
                     "fx_rate":fxr,"markup_pct":mkup,"qty":qty})
    ...
    for p in st.session_state.pricing:
        ...
        rows.append({
            ...
            "Cost USD":fmt_usd(p["cost_usd"]),"Cost ZMW":fmt_zmw(c["cost_zmw"]),
            "Markup %":f"{fmt_n(p['markup_pct'])}%" if item.get("type")!="recovery" else "—",
            "Selling USD":fmt_usd(c["selling_usd"]),"Selling ZMW":fmt_zmw(c["selling_zmw"]),
            "Margin/Unit":fmt_usd(c["margin_unit"]),"Total Margin":fmt_usd(c["total_margin"]),
            "Margin %":f"{fmt_n(c['margin_pct'])}%","Qty":f"{int(p['qty']):,}",
        })
    if rows: st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
    else: st.info("No pricing records yet.")
...
# ── INVOICES ──────────────────────────────────────────────────────────────────
            ...
            if st.form_submit_button("Save Invoice", type="primary"):
                if not supplier or not inv_num:
                    ...
                    st.session_state.invoices.append({
                        "id":nid(),"item_id":str(opts[sel]),"supplier":supplier,
                        "number":inv_num,"date":str(inv_date),"currency":currency,
                        "gross":gross,"vat":vat,"vat_rec":vat_rec,"qty":qty})
    ...
    for inv in st.session_state.invoices:
        ...
        rows.append({
            ...
            "Item":item.get("name","—"),"Date":inv["date"],"Ccy":inv["currency"],
            "Gross":fmt_n(inv["gross"]),"VAT Rec?":"Yes" if inv["vat_rec"] else "No",
...
# ── SAGE ACTUALS ──────────────────────────────────────────────────────────────
    ...
    st.markdown('<div class="info-box">ℹ️ When a ZMW actual matches a USD invoice, Implied FX is auto-calculated: Sage ZMW ÷ Invoice USD</div>', unsafe_allow_html=True)
    ...
    with st.expander("➕ Capture Sage Actual"):
        with st.form("add_actual"):
            ...
            with c1:
                sel      = st.selectbox("Item",list(opts.keys()))
                sage_doc = st.text_input("Sage Document # *")
            with c2:
            ...
            if st.form_submit_button("Save Actual", type="primary"):
                if not sage_doc:
                    ...
                    st.session_state.actuals.append({
                        "id":nid(),"item_id":str(opts[sel]),"sage_doc":sage_doc,
    ...
    if rows: st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
    else: st.info("No actuals yet.")

# ── FX RATES ──────────────────────────────────────────────────────────────────
elif page == "💱 FX Rates":
    st.title("💱 FX Rate Management")
    fx     = st.session_state.fx
    spread = float(fx["reporting"]) - float(fx["purchase"])
    ...
    with st.form("fx_form"):
        c1,c2,c3 = st.columns(3)
        with c1: new_p  = st.number_input("Purchase FX", value=float(fx["purchase"]), step=0.001,format="%.4f")
        with c2: new_pr = st.number_input("Pricing FX",  value=float(fx["pricing"]),  step=0.001,format="%.4f")
        with c3: new_r  = st.number_input("Reporting FX",value=float(fx["reporting"]),step=0.001,format="%.4f")
    ...
    st.divider()
    st.markdown("#### FX Scenario Analysis  *(±2 ZMW on Reporting FX)*")
    orig   = float(st.session_state.fx["reporting"])
    base_m = get_metrics()
    scen   = []
...
# ── RECONCILIATION ────────────────────────────────────────────────────────────
    ...
    st.markdown('<div class="info-box">ℹ️ Recovery items use Selling − Actual Cost (not commercial cost) to avoid distorting markup dashboards.</div>', unsafe_allow_html=True)
    ...
    for p in st.session_state.pricing:
        ...
        rows.append({
            ...
            "Comm Margin USD": fmt_usd(c["total_margin"]),
            "Actual Cost USD": fmt_usd(act_cost)   if act_cost   is not None else "—",
            "Actual Margin":   fmt_usd(act_margin)  if act_margin is not None else "—",
    ...
    with c4: metric_card("FX/Proc Var", f"{s} {fmt_usd(abs(totals['variance']))}",
                         color="green" if totals["variance"]>=0 else "red")
...
# ── REPORTS ───────────────────────────────────────────────────────────────────
    ...
    act_cost   = m["revenue"] - m["actual_margin"]
    ...
    with c1:
        ...
        st.dataframe(pd.DataFrame({
            ...
            "Amount":[fmt_usd(m["revenue"]),f"({fmt_usd(comm_cost)})",
                      fmt_usd(m["comm_margin"]),f"({fmt_usd(act_cost)})",
    ...
    with c2:
        ...
        st.dataframe(pd.DataFrame({
            ...
            "Amount":[fmt_n(rfx,3),fmt_zmw(m["revenue"]*rfx),f"({fmt_zmw(comm_cost*rfx)})",
                      fmt_zmw(m["comm_margin"]*rfx),f"({fmt_zmw(act_cost*rfx)})",
    ...
    for p in st.session_state.pricing:
        item = get_item(p["item_id"]); c = calc_pricing(p)
    ...
    for inv in st.session_state.invoices:
        item = get_item(inv["item_id"]); c = calc_invoice(inv)
